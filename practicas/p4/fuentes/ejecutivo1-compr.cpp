...
int main( int argc, char *argv[] )
{
   ...
   while( true ) // ciclo principal
   {
      ...
      for( int i = 1 ; i <= 4 ; i++ ) // ciclo secundario (4 iteraciones)
      {
         ...
         cout << "El instante final esperado es " << Ts.count() << " milisegundos" <<endl;
         

         // esperar hasta el inicio de la siguiente iteración del ciclo secundario
         sleep_until( ini_sec );
         
         float duracion = milliseconds_f(steady_clock::now()-ini_sec).count();

         cout << endl
              << "Medición de duración == " << duracion << " milisegundos." << endl ;
            if(duracion >20){
              cout << " Me salgo" <<endl;
              exit(1);
            }
